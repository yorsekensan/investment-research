...
with st.expander("📖 Backtester User Guide & Interpretation Guide", expanded=False):
    ...
    * **Target Asset:** The ticker you want to evaluate (Standard assets use 200-SMA, Transitional assets like FORU/DOOH automatically use Smart Money OBV logic).
    ...
    * **Overlay Relationship:**
        ...
        * **Inverse:** A **falling** macro indicator adds **+25%** to the score (e.g., PACK gains from strong IDR; Tech gains from dropping TNX).
...
with col1:
    ticker = st.selectbox("Target Asset", ["PACK.JK", "ADRO.JK", "BBCA.JK", "FORU.JK", "DOOH.JK", "BTC-USD", "GC=F"])
with col2:
    ...
    with st.spinner(f"Crunching {timeframe} historical data for {ticker}..."):
        try:
            ...
            if isinstance(data.columns, pd.MultiIndex):
                ...
                s_vol = data['Volume'][ticker].dropna() if 'Volume' in data.columns.levels[0] else pd.Series(dtype=float)
                macro_series = data['Close'][macro_ticker].dropna()
            ...
            is_transitional = ticker in ["FORU.JK", "DOOH.JK"]
            ...
            for i in range(len(df)):
                ...
                # --- MACRO EVALUATION (Applies to all) ---
                ...
                # --- SECTOR EVALUATION (Applies to all) ---
                ...
                if is_transitional:
                    ...
                    sub_obv = sub['OBV']
                    if len(sub_obv) >= 20 and sub_obv.iloc[-1] > sub_obv.rolling(20).mean().iloc[-1]: score += 20

                # --- STANDARD LOGIC (BBCA, ADRO, PACK, BTC, GOLD) ---
                else:
            ...
            with tab2:
                ...
                title_suffix = "(Transitional OBV Logic)" if is_transitional else "(Standard 200-SMA Logic)"
                fig.update_layout(template="plotly_dark", height=550, title=f"{ticker} {timeframe} Macro Regime Overlay {title_suffix}", margin=dict(l=0, r=0, t=40, b=0))
